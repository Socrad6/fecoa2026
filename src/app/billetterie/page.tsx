...
export default function BilletteriePage() {
  ...
  const inputCls = "border text-sm outline-none focus:border-gold transition-colors"
    ...
    <section className="py-[clamp(70px,9vh,128px)] px-[clamp(16px,5vw,80px)] pt-32">
      <div className="max-w-[1400px] mx-auto">
        <div className="mb-10">
          ...
          <p className="text-[clamp(13px,1.4vw,15px)] max-w-[520px] leading-[1.8]" style={{ color: 'var(--text-muted)' }}>
            Choisissez votre formule et réservez votre place pour la FÉCOA 2026.
          </p>
        </div>

        {/* Step indicator */}
        <div className="flex items-center gap-0 mb-12 border overflow-hidden max-w-[480px]" style={{ borderColor: 'var(--border)' }}>
          {[{ key: 'select', num: '01', label: 'Sélection' }, { key: 'cart', num: '02', label: 'Panier' }, { key: 'payment', num: '03', label: 'Paiement' }].map(s => (
            <button key={s.key} onClick={() => { if (s.key === 'select' || (s.key === 'cart' && totalItems > 0) || (s.key === 'payment' && totalItems > 0)) setStep(s.key as typeof step) }}
              className={`flex-1 flex items-center gap-3 px-5 py-3.5 transition-all duration-300 ${step === s.key ? 'bg-gold text-navy' : 'hover:bg-[rgba(200,155,60,.04)]'}`}
        ...
        {step === 'select' && (
          <>
            <div className="flex gap-3 flex-wrap mb-8">
              {promos.map(p => (
                <div key={p.label} className="flex items-center gap-2.5 px-4 py-2 border bg-[rgba(200,155,60,.04)]" style={{ borderColor: 'var(--border)' }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-[2px] border" style={{ borderColor: 'var(--border)' }}>
              {tickets.map(ticket => (
                <div key={ticket.id} className={`p-7 relative group ${ticket.popular ? 'ring-1 ring-gold/30' : ''}`} style={{ background: 'var(--bg-2)' }}>
                  ...
                  {ticket.popular && <span className="absolute top-3 right-3 text-[8px] font-bold tracking-[1.5px] uppercase px-2 py-0.5 text-navy" style={{ background: ticket.color }}>Populaire</span>}
                  <div className="mb-4">
                    <div className="text-[9px] font-bold tracking-[3px] uppercase mb-2" style={{ color: ticket.color }}>{ticket.name}</div>
                    <div className="font-display text-[clamp(26px,2.5vw,38px)] font-bold leading-none mb-1" style={{ color: 'var(--text-main)' }}>
                      {ticket.price === 0 ? 'Gratuit' : <>{ticket.price} <small className="text-[12px] font-normal" style={{ color: 'var(--text-muted)' }}>CAD</small></>}
                    </div>
                    <p className="text-[11px] mt-1" style={{ color: 'var(--text-muted)' }}>{ticket.description}</p>
                  </div>
                  <ul className="mb-5 pt-4 border-t" style={{ borderColor: 'var(--border)' }}>
                    {ticket.features.map(f => (
                      <li key={f} className="text-[11px] py-1 flex gap-2.5 items-center" style={{ color: 'var(--text-main)' }}>
        ...
        {step === 'cart' && (
          <div className="max-w-[640px]">
            <h3 className="font-display text-[22px] font-bold mb-6" style={{ color: 'var(--text-main)' }}>Votre panier</h3>
            {totalItems === 0 ? (
              <div className="text-center py-16 border" style={{ borderColor: 'var(--border)' }}>
                <p className="mb-3" style={{ color: 'var(--text-muted)' }}>Votre panier est vide.</p>
              ...
              <>
                <div className="border divide-y" style={{ borderColor: 'var(--border)' }}>
                  {Object.entries(cart).map(([id, qty]) => {
                    ...
                    return (
                      <div key={id} className="flex items-center justify-between p-4" style={{ background: 'var(--bg-2)' }}>
                        <div className="flex-1"><span className="text-[12px] font-semibold text-gold-2">{t.name}</span><span className="text-[11px] ml-2" style={{ color: 'var(--text-muted)' }}>{t.price === 0 ? 'Gratuit' : `${t.price} CAD / pièce`}</span></div>
                        <div className="flex items-center gap-4">
                          <div className="flex items-center gap-2">
                            <button onClick={() => removeFromCart(id)} className="w-7 h-7 border text-sm flex items-center justify-center transition-colors hover:border-gold hover:text-gold-2" style={{ borderColor: 'var(--border)', color: 'var(--text-muted)' }}>−</button>
                            <span className="text-[13px] w-6 text-center font-medium" style={{ color: 'var(--text-main)' }}>{qty}</span>
                            <button onClick={() => addToCart(id)} className="w-7 h-7 border text-sm flex items-center justify-center transition-colors hover:border-gold hover:text-gold-2" style={{ borderColor: 'var(--border)', color: 'var(--text-muted)' }}>+</button>
                          </div>
                          <span className="text-[13px] text-gold-2 font-semibold w-20 text-right">{t.price * qty} CAD</span>
                        </div>
                      </div>
                    )
                  })}
                ...
                <div className="flex gap-2 mb-6 mt-4">
                  <input type="text" value={promoCode} onChange={e => setPromoCode(e.target.value)} placeholder="Code promo" className={`flex-1 px-4 py-2.5 ${inputCls}`} style={inputStyle} />
                ...
                <div className="border-t pt-4 space-y-2" style={{ borderColor: 'var(--border)' }}>
                  <div className="flex justify-between text-[13px]" style={{ color: 'var(--text-muted)' }}><span>Sous-total</span><span>{subtotal} CAD</span></div>
                  {discount > 0 && <div className="flex justify-between text-[13px] text-[#4ade80]"><span>Remise</span><span>-{discount} CAD</span></div>}
                  <div className="flex justify-between text-lg font-display font-bold text-gold-2 pt-3 border-t" style={{ borderColor: 'var(--border)' }}><span>Total</span><span>{total} CAD</span></div>
                </div>
        ...
        {step === 'payment' && (
          <div className="max-w-[640px]">
            <h3 className="font-display text-[22px] font-bold mb-6" style={{ color: 'var(--text-main)' }}>Paiement sécurisé</h3>
            <div className="p-6 mb-6 border" style={{ background: 'var(--bg-2)', borderColor: 'var(--border)' }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <input type="text" placeholder="Prénom *" required value={customerInfo.firstName} onChange={e => setCustomerInfo({...customerInfo, firstName: e.target.value})} className={`px-3.5 py-2.5 ${inputCls}`} style={inputStyle} />
                <input type="text" placeholder="Nom *" required value={customerInfo.lastName} onChange={e => setCustomerInfo({...customerInfo, lastName: e.target.value})} className={`px-3.5 py-2.5 ${inputCls}`} style={inputStyle} />
                <input type="email" placeholder="Courriel *" required value={customerInfo.email} onChange={e => setCustomerInfo({...customerInfo, email: e.target.value})} className={`px-3.5 py-2.5 ${inputCls} md:col-span-2`} style={inputStyle} />
                <input type="tel" placeholder="Téléphone (optionnel)" value={customerInfo.phone} onChange={e => setCustomerInfo({...customerInfo, phone: e.target.value})} className={`px-3.5 py-2.5 ${inputCls} md:col-span-2`} style={inputStyle} />
              </div>
            </div>
            <div className="mb-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-[2px] border" style={{ borderColor: 'var(--border)' }}>
                {[{ id: 'stripe', name: 'Stripe', icon: '💳', desc: 'Visa / Mastercard' }, { id: 'paypal', name: 'PayPal', icon: '🅿️', desc: 'Compte PayPal' }, { id: 'orange_money', name: 'Orange Money', icon: '🟠', desc: 'Mobile Money' }, { id: 'wave', name: 'Wave', icon: '🔵', desc: 'Mobile Money' }].map(m => (
                  <button key={m.id} onClick={() => setSelectedPayment(m.id)} className={`flex items-center gap-3 p-4 transition-all text-left cursor-pointer ${selectedPayment === m.id ? 'bg-[rgba(200,155,60,.08)] border border-gold' : 'border border-transparent hover:bg-[rgba(200,155,60,.04)]'}`}
                    style={selectedPayment !== m.id ? { background: 'var(--bg-2)' } : {}}>
                    <span className="text-xl">{m.icon}</span>
                    <div className="flex-1"><span className="block text-[12px] font-semibold" style={{ color: 'var(--text-main)' }}>{m.name}</span><span className="text-[10px]" style={{ color: 'var(--text-muted)' }}>{m.desc}</span></div>
                    {selectedPayment === m.id && <span className="text-gold text-sm">✓</span>}
                  </button>
                ))}
              </div>
            </div>
            <div className="p-5 mb-6 border" style={{ background: 'var(--bg-2)', borderColor: 'var(--border)' }}>
              <h4 className="text-[11px] font-bold tracking-[2px] uppercase text-gold-2 mb-3">Récapitulatif</h4>
              {Object.entries(cart).map(([id, qty]) => { const t = tickets.find(t => t.id === id)!; return <div key={id} className="flex justify-between text-[12px] py-1.5" style={{ color: 'var(--text-main)' }}><span>{qty}x {t.name}</span><span>{t.price * qty} CAD</span></div> })}
              {appliedPromo && <div className="flex justify-between text-[12px] text-[#4ade80] py-1.5"><span>Remise</span><span>-{discount} CAD</span></div>}
              <div className="flex justify-between text-lg font-display font-bold text-gold-2 mt-3 pt-3 border-t" style={{ borderColor: 'var(--border)' }}><span>Total</span><span>{total} CAD</span></div>
            </div>
            <p className="text-[11px] mb-4 flex items-center gap-2" style={{ color: 'var(--text-muted)' }}>
