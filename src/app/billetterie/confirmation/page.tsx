    ...
    <section className="py-[clamp(70px,9vh,128px)] px-[clamp(16px,5vw,80px)] pt-32 min-h-screen">
      <div className="max-w-[600px] mx-auto text-center">
        {loading ? (
          ...
          <div className="py-20 border border-[rgba(200,155,60,.08)]">
            ...
            <h2 className="font-display text-[26px] font-bold text-white mb-3">Erreur</h2>
            <p className="text-muted mb-6 text-[14px]">{error}</p>
          ...
          <>
            ...
            <div className="mb-8">
              ...
              <h2 className="font-display text-[28px] font-bold text-white mb-2">Merci pour votre achat !</h2>
            ...
            {/* Order summary */}
            <div className="bg-navy-2 border border-[rgba(200,155,60,.08)] p-6 mb-5 text-left">
              <div className="flex justify-between items-center mb-4">
                <span className="text-[10px] font-bold tracking-[3px] uppercase text-gold-2">
                  Commande #{order.orderId?.slice(-8).toUpperCase()}
                </span>
                <span className={`text-[9px] font-bold tracking-[2px] uppercase px-2.5 py-0.5 rounded-full ${
                  order.status === 'paid' ? 'bg-[rgba(26,122,60,.15)] text-[#4ade80]' : 'bg-[rgba(200,155,60,.14)] text-gold-2'
                }`}>
                  {order.status === 'paid' ? 'Payée' : 'En attente'}
                </span>
              </div>

              <div className="divide-y divide-[rgba(200,155,60,.06)]">
              ...
              {order.discount > 0 && (
                <div className="flex justify-between py-2 text-[13px] border-t border-[rgba(200,155,60,.06)]">
            ...
            {order.tickets?.length > 0 && (
              <div className="bg-navy-2 border border-[rgba(200,155,60,.08)] p-6 mb-6 text-left">
                <h3 className="text-[11px] font-bold tracking-[2px] uppercase text-gold-2 mb-4">Vos billets</h3>
                <div className="space-y-3">
                  {order.tickets.map((ticket) => (
                    <div key={ticket.id} className="flex items-center justify-between p-3 bg-[rgba(255,255,255,.02)] border border-[rgba(200,155,60,.06)]">
                      <div>
                        <span className="text-[13px] text-text font-medium">{ticket.type}</span>
                        <span className="block text-[10px] text-muted mt-0.5">QR: {ticket.qrCode?.slice(0, 12)}...</span>
                      </div>
