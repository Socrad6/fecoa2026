    ...
    <>
      <section className="py-[clamp(70px,9vh,128px)] px-[clamp(16px,5vw,80px)] pt-32 bg-navy-2">
        <div className="max-w-[1400px] mx-auto">
          ...
          {/* Stands grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-[2px] bg-[rgba(200,155,60,.12)] border border-[rgba(200,155,60,.12)]">
            {stands.map(s => (
              <div key={s.tier} className={`bg-navy-2 p-7 relative group ${s.popular ? 'bg-gradient-to-br from-[rgba(14,45,74,.85)] to-[rgba(6,21,36,.85)] border border-[rgba(200,155,60,.2)]' : ''}`}>
                ...
                <div className="font-display text-[clamp(26px,2.5vw,36px)] font-bold text-white leading-none mb-1">
                  {s.price === 'Sur devis' ? <>Sur <small className="text-[12px] text-muted font-normal">devis</small></> : <>{s.price} <small className="text-[12px] text-muted font-normal">CAD</small></>}
                </div>
                <p className="text-[11px] text-muted my-2 pb-3 border-b border-[rgba(200,155,60,.08)]">{s.size}</p>
                <ul className="mb-5">
                  {s.features.map(f => (
                    <li key={f} className="text-[11px] text-text py-1.5 flex gap-2.5 items-center">
          ...
          {/* Promos */}
          <div className="mt-6 flex items-center gap-4 p-4 border border-[rgba(200,155,60,.12)] bg-[rgba(200,155,60,.04)] flex-wrap">
            <div className="flex items-center gap-2.5 flex-1 min-w-[200px]">
              <span className="text-[8px] font-bold tracking-[1px] uppercase px-2.5 py-1 bg-gold text-navy">Early Bird</span>
              <p className="text-[12px] text-text"><strong className="text-gold-2">−15%</strong> avant le <strong>1er Nov 2026</strong></p>
            </div>
            ...
            <div className="flex items-center gap-2.5 flex-1 min-w-[200px]">
              ...
              <p className="text-[12px] text-text"><strong className="text-gold-2">−10%</strong> au Canada <em className="text-muted text-[10px]">(cumulables)</em></p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-[clamp(50px,6vh,80px)] px-[clamp(16px,5vw,80px)]">
        <div className="max-w-[1400px] mx-auto">
          <h3 className="font-display text-[22px] font-bold text-white mb-2">Secteurs d&apos;activité représentés</h3>
          <p className="text-[13px] text-muted mb-6">Les principales industries de l&apos;Afrique de l&apos;Ouest seront présentes.</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-[2px]">
            {categories.map(c => (
              <div key={c} className="px-5 py-3 border border-[rgba(200,155,60,.08)] text-[12px] text-text hover:border-gold hover:text-gold-2 hover:bg-[rgba(200,155,60,.03)] transition-all cursor-default">
                {c}
              </div>
            ))}
          </div>
